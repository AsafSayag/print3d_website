...
/** Monochrome brand glyphs (24×24, filled with currentColor). */
const ICONS: Record<SocialId, React.ReactNode> = {
  ...
  google: (
    <path d="M12.48 10.92v3.28h7.84c-.24 1.84-.85 3.18-1.79 4.13-1.14 1.14-2.93 2.4-6.05 2.4-4.83 0-8.6-3.89-8.6-8.72s3.77-8.72 8.6-8.72c2.6 0 4.51 1.02 5.91 2.35l2.31-2.31C18.75 1.44 16.13 0 12.48 0 5.87 0 .31 5.39.31 12s5.56 12 12.17 12c3.57 0 6.27-1.17 8.37-3.36 2.16-2.16 2.84-5.21 2.84-7.67 0-.76-.05-1.47-.17-2.05H12.48Z" />
  ),
};

/**
 * Row of round social-profile icons driven off `SOCIAL_LINKS`. A profile with
 * no URL yet renders as an inert icon (an `<a>` without `href` is neither
 * clickable nor focusable), so the row's layout is final before the links are.
 */
export function SocialLinks({
  location,
  className = "",
}: {
  /** Analytics surface identifier, e.g. `"footer"`. */
  location: string;
  className?: string;
}) {
  return (
    <ul className={`flex items-center gap-3 ${className}`} aria-label="רשתות חברתיות">
            ...
            <a
              {...(href
                ? {
                    ...
                    ...analyticsAttrs("social_click", { platform: id, location }),
                  }
                : {})}
              aria-label={`Print3D ב־${label}`}
              title={label}
              className="grid h-9 w-9 place-items-center rounded-full border border-white/15 bg-white/[0.04] text-white/75 transition-colors hover:border-white/35 hover:bg-white/10 hover:text-white"
            >
